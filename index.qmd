...
The code block below demonstrate the penguins dataset that we will be using and shows the correlation between body_mass_g and flipper_legth_mm that we will be using to construct a model before and after imputation of missing data.
...
In the code block below, we will create 20% missing at random data in the column flipper_length_mm. Then we will create a linear regression to display the r-squared that would be achieved without performing any imputation on the data set.
...
summary(regg)$adj.r.squared

```

Now we will proceed to impute median values into the missing data set for bill_length_mm and construct the same model to achieve an increase in r-squared.
